...
面向~\ref{eq:consistency}中的No Conflict公理，上述约束实现了它的逆否命题，从而保证了无冲突公理的正确性。
\begin{equation}
\mathsf{NoConflict}(T) \equiv \forall T' \in \mathcal{T}.
\ T' \centernot{\xrightarrow{\text{VIS}}} T \Rightarrow \neg(T' \bowtie T) \lor T' \centernot{\xrightarrow{\text{AR}}} T
\end{equation}
