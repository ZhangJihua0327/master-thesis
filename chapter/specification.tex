...
最后，本章基于这些公理定义了六个隔离级别，从弱到强大致依次为读原子性（Read Atomic，RA）、因果一致性（Causal Consistency，CC）、并发快照隔离（Parallel Snapshot Isolation，PSI）、前缀一致性（Prefix Consistency，PC）、快照隔离（Snapshot Isolation，SI）和可串行化（Serialisability，SER），并分析了各级别之间的偏序关系及其能够防止或允许的异常类型。
...
快照（Snapshot，记为 \Snapshot），是数据库在指定时间戳上创建的只读一致性数据视图，是分布式事务一致性的核心底层机制。本文主要讨论的是事务级别的快照。快照是实现多版本并发控制（Multi-Version Concurrency Control，MVCC）的基础，通过为每个事务分配一个独立的快照，允许多个事务并发执行而不会相互干扰，从而提高系统的并发性能和响应速度。对于一个事务，我们不需要将数据完全复制一遍，而是确定一个版本时间戳（readTimestamp、commitTimestamp）并基于定义的规则：只可见该时间戳之前已提交的所有数据版本，不可见之后的任何修改；数据仍然存在原地，只是通过版本号做 “可见性过滤”。降低开销的基础上，为事务的并发提供条件。快照函数 $S$ 将键映射到其对应的值，表示在该快照下每个键的值。
...
\section{本章小结}
本章构建了分布式数据库一致性模型的形式化描述框架。
首先，本章阐述了在存在网络延迟和故障的分布式环境下，进行一致性建模的必要性，指出了一致性模型是数据库与应用之间的重要契约。
其次，本章给出了事务、历史、快照等基本概念的形式化定义，并引入了可见性关系 ($\text{VIS}$) 和仲裁关系 ($\text{AR}$) 两个核心关系，提出了 $\text{VIS} \subseteq \text{AR}$ 的一致性基础。
在此基础上，本章详细分析了因果违背、破碎读、脏读、更新丢失、长叉异常和写偏斜等数据一致性异常，并提出了内部一致性、外部一致性、传递可见性、前缀一致性、无冲突和完全可见性等六条一致性公理。
最后，本章基于定义的公理体系，形式化地定义了六种隔离级别：读原子性 (RA)、因果一致性 (CC)、并发快照隔离 (PSI)、前缀一致性 (PC)、快照隔离 (SI) 和可串行化 (SER)，并系统地比较了它们之间的偏序关系以及对各种异常的防御能力。
这一形式化框架为后续章节中具体的事务处理协议设计验证提供了理论基础。
